...
public class QueueUIPage extends InteractiveCustomUIPage<QueueUIPage.QueueData> {
    ...
    public void handleDataEvent(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull QueueData data) {
        ...
        switch (data.button) {
            ...
            case "Next" -> {
                player.getPageManager().openCustomPage(ref, store, new TeamColorSelectUIPage(playerRef, thisMap, null)); // TODO: MAKE SURE TO ADD TO THE NEW UI'S CONSTRUCTOR: BedwarsMap map, COPY OVER thisMap.
            }
